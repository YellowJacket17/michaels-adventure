...
public class EventManager {
    ...
    public void addEntityToParty(int entityId) {
        ...
        if (gp.getParty().size() <= gp.getNumActivePartyMembers()) {
            ...
            setEntityFollowTarget(entityId, gp.getPlayer().getEntityId());                                              // Entity is in active party, so set to follow player entity.
        } else {
     ...
     * @param show sets removed party members to visible (true) or retains existing hidden status (false)
     */
    public void removeEntityFromParty(int entityId, boolean show) {
        ...
        breakFollowerChain(gp.getPlayer());                                                                             // Break chain of party entities following player entity (will rebuild later).
        ...
        for (EntityBase entity : gp.getParty().values()) {                                                              // Set party entity world positions and directions to match those of party entities originally at said index of party map.
            ...
            if (entityIndex < gp.getNumActivePartyMembers()) {
                ...
                setEntityFollowTarget(entity.getEntityId(), gp.getPlayer().getEntityId());                              // Set active party members as following player entity.
    ...
    public void swapEntityInParty(int swapperEntityId, int swappeePosition) {
        ...
        for (int j = 0; j < originalParty.size(); j++) {                                                                // Add party entities back to party map in new ordering.
            ...
            if (j == swapperPosition) {                                                                                 // Position/index to move secondary entity to.
                ...
                gp.getParty().put(originalParty.get(swappeePosition).getEntityId(), originalParty.get(swappeePosition));
            } else if (j == swappeePosition) {                                                                          // Position/index to move primary entity to.

                gp.getParty().put(originalParty.get(swapperPosition).getEntityId(), originalParty.get(swapperPosition));
     ...
     *
     * @param showAll sets all removed party members to visible (true) or retains their existing hidden status (false)
     */
    public void removeAllEntitiesFromParty(boolean showAll) {

        breakFollowerChain(gp.getPlayer());
